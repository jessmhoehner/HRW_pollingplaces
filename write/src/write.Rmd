...
az_t3 <- table(az$gp_black, az$delta_cat)
...
or lost polling locations. ex: In zip codes where between 0-23 people
identified as Hispanic or Latino, 7 gained polling locations, 9 lost polling
locations, and 81 maintained polling locations. Zip codes may overlap between 
demographic groups such that one zip code could have between 0-23 people
who identify as Hispanic or Latino and between 0-1210 people who identify as White.


```{r sckables, echo=FALSE, message=FALSE, paged.print=TRUE}
...
(n = `r nrow(az_dcg_ndmaricopa)`), `r comma(sum(az_dcg_ndmaricopa$total_hl))` people identified 
as Hispanic or Latino, `r comma(sum(az_dcg_ndmaricopa$nhl_white))` people identified 
as White, `r comma(sum(az_dcg_ndmaricopa$nhl_black))` people identified as 
Black or African-American, `r comma(sum(az_dcg_ndmaricopa$nhl_ai_an))` people identified 
as American Indian or Alaskan Native, `r comma(sum(az_dcg_ndmaricopa$nhl_asian))` people 
identified as Asian, `r comma(sum(az_dcg_ndmaricopa$nhl_nhi_pi))` people identified 
as Native Hawaiian or Pacific Islander, and `r comma(sum(az_dcg_ndmaricopa$nhl_ao))`
people identified as some other race or two or three or more races.

In those zip codes without complete COVID-19 data which maintained polling locations 
(n = `r nrow(az_dcm_ndmaricopa)`),`r comma(sum(az_dcm_ndmaricopa$total_hl))` people identified 
as Hispanic or Latino, `r comma(sum(az_dcm_ndmaricopa$nhl_white))` people identified 
as White, `r comma(sum(az_dcm_ndmaricopa$nhl_black))` people identified as 
Black or African-American, `r comma(sum(az_dcm_ndmaricopa$nhl_ai_an))` people identified 
as American Indian or Alaskan Native, `r comma(sum(az_dcm_ndmaricopa$nhl_asian))` people 
identified as Asian, `r comma(sum(az_dcm_ndmaricopa$nhl_nhi_pi))` people identified 
as Native Hawaiian or Pacific Islander, and `r comma(sum(az_dcm_ndmaricopa$nhl_ao))`
people identified as some other race or two or three or more races.
